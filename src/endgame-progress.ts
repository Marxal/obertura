...
// attempts always increments; lastTrained stamps now.
export function foldResult(prev: EndgameRecord | undefined, solved: boolean, now: number): EndgameRecord {
  return {
...
// Record one play-out result and persist it.
export function recordEndgameResult(id: string, solved: boolean, now: number = Date.now()): EndgameRecord {
  const p = load();
  const next = foldResult(p[id], solved, now);
  p[id] = next;
  save(p);
  return next;
}
