# Bito Chess — roadmap history (v1.0 → v0.6d)

Every round that shipped from the first working trainer up to just before the
Stripe migration. Split out of `ROADMAP.md` so the working roadmap stays short;
the live file keeps the recent rounds and a one-line index of everything here.

**All of it shipped.** Every `## ` heading below is ✅.

## About the version numbers

The `v0.x` numbers in these headings were **planning labels, not releases**, and
they are not a clean sequence. Two things to know before trusting them:

- **The project renumbered mid-flight.** It ran an internal `v1.x` scheme, then
  moved to a public `v0.x` beta scheme — conceptually v1.0→v0.1, v1.1→v0.2,
  v1.2→v0.2, v1.3→v0.3. That is why `v1.0`–`v1.3` and `v0.4`+ both appear here.
- **`v0.6` was used twice**, for two unrelated rounds. They are disambiguated
  below as **`v0.6 (cloud backup)`** — the Google Drive round, which came right
  after v0.5 — and **`v0.6 (redesign)`**, the repertoire data-model redesign that
  came *after* v0.30 and continued as v0.6b/c/d. The numbers were left as they
  were written rather than retro-fitted into a tidy sequence, because rewriting
  them would invent a history that did not happen.

The numbers that are real are the **git tags**: `v0.3`, `v0.4`, `v0.5`, plus the
legacy `v1.0`–`v1.3`.

Sections are in the order they were written, which is not strictly chronological
— `v1.0`–`v1.3` sit after `v0.4`/`v0.5`, and the redesign group sits after
`v0.30`. The index in `ROADMAP.md` lists them in the order they actually shipped.

Two bullets inside the v0.10 endgame round are still marked 🔜. Those were
forward-looking notes written at the time about later work, and they are left as
written rather than flipped, because whether they shipped was not verified.
...
## v0.6 (cloud backup) — cloud backup & publishing groundwork ✅
...
## v0.6 (redesign) — the repertoire redesign ✅
