...
export const Icons = {
  ...
  // The puzzle clock's own "hide" state — the same face, slashed through
  // (matching eyeOff's pattern), so the toggle reads as "clock, off" rather
  // than borrowing the generic eye glyph for a control that only hides a clock.
  clockOff: (s?: number) => svg(`<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/><path d="m2 2 20 20"/>`, s),
